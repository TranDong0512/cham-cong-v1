        ...
        delay(async () => {
            try {
                ...
                if (img.landmark.length > 0) {
                    ...
                    if (imgSimilarity?.value !== undefined) {
                        let imgUrl = await ExportImg(
                            img,
                            landMarkPosition.boundingBox
                        );
                        if (imgSimilarity.direction === currentDirection) {
                            imgArr.push({
                                img: imgUrl,
                        ...
                        if (currentDer.length === 5) {
                            ...
                            newHidden[currentPos] = true;
                            setImgOkPositions(subDir[currentPos]);
                            setHiddenStates([...newHidden]);
                            if (currentPos < 8) {
                                setRobotTitle(botContent[currentPos + 1]);
                                speech(
                                    `Thành công, Hướng tiếp theo: ${
                                        subDir[currentPos + 1]
                                    }`
                                );
                                currentPos++;
