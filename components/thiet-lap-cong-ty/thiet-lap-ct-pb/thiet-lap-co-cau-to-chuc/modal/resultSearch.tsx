...
import { managerUser } from "@/components/commons/constant";
...
export default function ResultSearch({ result, setResult, setManagerUser }) {
  ...
  useEffect(() => {
    const fetcher = async () => {
      return await instance.post(managerUser.listUser);
    };
    const unFollow = async () => {
      try {
        const data = await fetcher();
        const option = data?.data?.data?.data?.map((item) => ({
          ep_name: item.userName,
          ep_image: item.avatarUser,
          dep_name: item.positionName,
          ep_id: item.ep_id,
          ep_phone: item.phone,
          key: item.ep_id,
        }));
